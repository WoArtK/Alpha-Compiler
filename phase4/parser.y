...
term: '(' expr ')' { $<exprval>$=$<exprval>expr;
	...
	| '-' expr {
		...
		$<exprval>$ =newexpr(arithmexpr_e);
					  ...
					  um->numConst=-1;
					  emit(mul,$<exprval>2,um, $<exprval>$, yylineno);
...
assignexpr: lvalue {
					...
					if(temp2!=NULL){
						...
						if ($<exprval>lvalue->type ==tableitem_e) {
							...
							printf("lvalue :%d index %d expr %d \n",$<exprval>lvalue->type, $<exprval>lvalue->index->type,$<exprval>expr->type);
...
call:  call '(' elist ')' { 
	...
	|lvalue callsuffix {printf("lvaluecallsuffix to call\n"); 
						if($<exprval>callsuffix->methodCalls.method==1){
							if(lookup_AllScopes($<exprval>lvalue->symbol->name,scope)->type==LIBFUNC)$<exprval>lvalue->type=libraryfunc_e;
							if(lookup_AllScopes($<exprval>lvalue->symbol->name,scope)->type==USERFUNC)$<exprval>lvalue->type=programfunc_e;
...
indexedelem:	'{' expr ':' expr '}'	{
				node *temp ;
				node *temp2;
				if($<exprval>2->symbol!=NULL)temp = lookup_AllScopes($<exprval>2->symbol->name,scope);
				if($<exprval>4->symbol!=NULL)temp2 = lookup_AllScopes($<exprval>4->symbol->name,scope);
			
				if(temp!=NULL){
					if(temp->type==LIBFUNC)$<exprval>2->type=libraryfunc_e;
				}
				if(temp2!=NULL){
					if(temp2->type==LIBFUNC)$<exprval>4->type=libraryfunc_e;
				}
