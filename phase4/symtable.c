...
void insert_libfunc(){
	...
	insertNode("print",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("input",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("objectmemberkeys",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("objecttotalmembers",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("objectcopy",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("totalarguments",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("typeof",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("strtonum",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("sqrt",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("cos",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("sin",0,0,LIBFUNC,currscopeoffset(),currscopespace());
	insertNode("argument",0,0,LIBFUNC,currscopeoffset(),currscopespace());
...
node * lookup_CertScope(char *name, int scope){
	...
	while(temp != NULL){
		if((strcmp(name,temp->name)==0) && (scope == temp->scope) && (temp->isActive == 1)){
...
node * lookup_Temp(char *name){
	...
	for(i; i<1000; i++){
		node * temp = HashTable[i];
		while(temp != NULL){
			if((temp->isActive == 0 ) && (temp->type==TMP)&& (strcmp(temp->name,name)==0 )){
